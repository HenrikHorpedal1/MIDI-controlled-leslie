...
1. "inertial mode" (default): A constant velocity is commanded (see above),
then deviations from this constant speed can be assumed to be caused by
encoder non-linearity.  This can be used even if there is only a single
encoder configured.
...
# `histogram` and `encoder_math` helper modules that ship with the moteus
# submodule under software/third_party/moteus/utils/.  Add that directory to
# the import path so the helpers resolve without copying them.
...
async def main():
    ...
    # Acceleration limit for the ramp up to --speed, in output rev/s^2.
    parser.add_argument('--accel', type=float, default=4.0)
    # Extra settle margin after the ramp before histogram capture, in seconds.
    parser.add_argument('--settle-time', type=float, default=1.0)
    # Histogram capture duration per split, in seconds.
    parser.add_argument('--sample-time', type=float, default=8.0)
